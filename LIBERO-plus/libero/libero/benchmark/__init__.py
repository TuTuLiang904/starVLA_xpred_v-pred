...
class Benchmark(abc.ABC):
    ...
    def _make_benchmark(self):
        ...
        # LIBERO-Plus task orders contain thousands of IDs; avoid printing the
        # full list for every evaluator process startup.
    ...
    def get_task_init_states_ori(self, i):
        ...
        # LIBERO initial states are trusted benchmark assets, not model
        # checkpoints.  PyTorch >= 2.6 defaults to weights_only=True, which
        # rejects their NumPy-backed serialized state format.
    ...
    def get_task_init_states(self, i):
        ...
        # See the matching compatibility note in the other task-suite loader.
        init_states = torch.load(init_states_path, weights_only=False)
        if "_add_" in self.tasks[i].init_states_file or "_level" in self.tasks[i].init_states_file:
