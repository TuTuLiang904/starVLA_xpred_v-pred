...
def _limit_cpu_threads_from_env() -> None:
    """Bound per-server PyTorch pools when multiple local eval jobs coexist."""
    value = os.getenv("STARVLA_CPU_THREADS")
    if not value:
        return
    try:
        threads = int(value)
        ...
        raise ValueError("STARVLA_CPU_THREADS must be a positive integer") from None

    # This must run before importing/building the framework; otherwise PyTorch
    # creates a large intra-op pool for every policy-server process.
    torch.set_num_threads(threads)
    torch.set_num_interop_threads(min(threads, 4))
    logging.info("Limited PyTorch CPU threads per policy server to %d", threads)


_limit_cpu_threads_from_env()
