...
class RoboTwinSubsetDataset(LeRobotSingleDataset):
    """A view over a subset manifest without duplicating RoboTwin parquet/videos."""

    def _get_trajectories(self):
        ids, lengths = super()._get_trajectories()
        manifest = self.dataset_path / "meta" / "episode_indices.json"
        if not manifest.is_file():
            raise FileNotFoundError(f"RoboTwin subset view needs {manifest}")
        with manifest.open(encoding="utf-8") as f:
            selected = np.asarray(json.load(f)["episode_indices"], dtype=ids.dtype)
        valid = np.isin(ids, selected)
        if valid.sum() != len(selected):
            raise ValueError(f"Subset manifest references unavailable episode IDs: {manifest}")
        return ids[valid], lengths[valid]


class RoboTwinSubsetDataConfig(AgilexData50Config):
    """50-step QwenGR00T data config for the public RoboTwin subset views."""
...
ROBOT_TYPE_CONFIG_MAP = {
    ...
    "robotwin_subset": RoboTwinSubsetDataConfig(),
}
...
DATASET_NAMED_MIXTURES = {
    # Zero-copy views generated by prepare_robotwin_dataset.py.
    # `full` preserves the official 50 clean + 500 randomized trajectories
    # per task.  Do not mix the two views at equal dataset weights here: that
    # would accidentally train at 1:1 instead of the intended 1:10 ratio.
    "robotwin_all_50_500": [
        ("full", 1.0, "robotwin_subset"),
    ],
    # Official 50-demonstration data-scaling setting: clean only.
    "robotwin_clean_50": [
        ("clean", 1.0, "robotwin_subset"),
    ],
    "robotwin_all": [
