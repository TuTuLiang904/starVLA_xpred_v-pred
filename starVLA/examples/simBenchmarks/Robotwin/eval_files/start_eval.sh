...
launch_task_in_slot() {
    ...
    (
        ...
        echo "[INFO] Policy server ready task=${task_name} gpu=${gpu_id} port=${port}"

        cd "${SCRIPT_DIR}"
        echo "[INFO] Starting RoboTwin eval task=${task_name} mode=${TASK_CONFIG}"
        bash "${SCRIPT_DIR}/eval.sh" \
            ...
            "${CKPT_PATH}" \
            "${port}" \
            > >(tee "${eval_log}" | grep --line-buffered -E "Task Name|Starting episode|Success!|Fail!|Success rate|Data has been saved|Error:|Traceback" | sed -u "s/^/[RESULT] ${task_name}: /") 2>&1
    ) &
