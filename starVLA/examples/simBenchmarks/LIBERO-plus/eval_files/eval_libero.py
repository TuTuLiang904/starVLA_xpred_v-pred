...
class Args:
    ...
    # LIBERO-Plus contains 10,030 tasks.  Saving every rollout video would
    # create tens of thousands of MP4s per checkpoint, so keep it opt-in for
    # full zero-shot evaluations.  Enable only for debugging/case studies.
    ...
    # Evaluate the half-open task-index interval [start_idx, end_idx).  The
    # defaults preserve the original behavior of evaluating the whole suite.
    start_idx: int = -1
    end_idx: int = -1
...
def eval_libero(args: Args) -> None:
    ...
    end_idx = num_tasks_in_suite if args.end_idx == -1 else args.end_idx
    if not (0 <= start_idx < end_idx <= num_tasks_in_suite):
        raise ValueError(
            f"Invalid task shard [{start_idx}, {end_idx}) for {args.task_suite_name} "
            f"with {num_tasks_in_suite} tasks."
        )
    logging.info("Evaluating task shard [%d, %d) of %d", start_idx, end_idx, num_tasks_in_suite)
    ...
    ID2CATEGORY = {}
    # A shard reports counts only for its own task indices, so separate shard
    # JSONs can be summed without double-counting category denominators.
    for item in TASK_MAPPING[start_idx:end_idx]:
    ...
    result_name = (
        f"{args.task_suite_name}.json"
        if start_idx == 0 and end_idx == num_tasks_in_suite
        else f"{args.task_suite_name}_{start_idx}_{end_idx}.json"
    )
