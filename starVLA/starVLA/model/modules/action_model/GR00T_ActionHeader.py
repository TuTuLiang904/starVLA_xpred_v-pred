...
# The GR00T flow path is x_t = (1 - t) * noise + t * clean_action.  Keep the
# output parameterisation and loss explicit so that v-pred, direct x-loss, and
# ABot AML-style x-pred/v-loss experiments share the same backbone, data, and
# noise path.
_PREDICTION_TYPE_ALIASES = {
    ...
    "x_pred": "x_prediction",
    "sample": "x_prediction",  # Hugging Face Diffusers' name for x0/sample prediction.
    # ABot AML: predict clean x0/action, then calculate the loss in velocity
    # space.  Keep this separate from x_prediction so existing direct x-MSE
...
def normalize_prediction_type(prediction_type) -> str:
    """Return a canonical GR00T output/loss experiment name.

    ``v_prediction`` preserves the released flow-matching baseline.  In
    both x-pred modes the DiT directly predicts the clean normalized action,
    i.e. the endpoint at t=1 of GR00T's linear flow path.  ``x_prediction``
    uses unweighted clean-action MSE, while ``x_prediction_v_loss`` implements
    ABot AML's velocity-space MSE.
    """
    key = str(prediction_type).lower()
    if key not in _PREDICTION_TYPE_ALIASES:
        supported = ", ".join(("v_prediction", "x_prediction", "x_prediction_v_loss"))
        raise ValueError(f"Unsupported prediction_type={prediction_type!r}; use one of: {supported}.")
    return _PREDICTION_TYPE_ALIASES[key]


def flow_matching_target(actions: torch.Tensor, noise: torch.Tensor, prediction_type: str) -> torch.Tensor:
    """Build the training target for GR00T's fixed linear flow path."""
    if prediction_type == "v_prediction":
        return actions - noise
    if prediction_type in ("x_prediction", "x_prediction_v_loss"):
        return actions
    raise ValueError(f"Unsupported canonical prediction_type={prediction_type!r}")
...
def flow_matching_loss(
    ...
    """Calculate the loss for the selected flow-matching parameterisation.

    ``x_prediction_v_loss`` is ABot's Action Manifold Learning objective.  It
    predicts clean actions but converts both prediction and target to the
    velocity field before MSE:

        ||(x_hat - x_t) / max(1 - t, t_eps)
          - (x_0 - x_t) / max(1 - t, t_eps)||^2.

    Algebraically this is a time-reweighted clean-action MSE, with weight
    1 / max(1 - t, t_eps)^2.  The explicit velocity form below mirrors
    AML_ActionHeader.py and avoids ambiguity about the intended objective.
    """
    if prediction_type == "v_prediction":
        return ((pred_actions - (actions - noise)) ** 2).mean()
    if prediction_type == "x_prediction":
        return ((pred_actions - actions) ** 2).mean()
    if prediction_type == "x_prediction_v_loss":
        denominator = (1 - time).clamp_min(t_eps)
        pred_velocity = (pred_actions - noisy_trajectory) / denominator
        velocity = (actions - noisy_trajectory) / denominator
        return ((pred_velocity - velocity) ** 2).mean()
    raise ValueError(f"Unsupported canonical prediction_type={prediction_type!r}")
...
def flow_matching_step(
    ...
    """Advance GR00T's deterministic linear flow from ``time`` to ``next_time``.

    For x-pred we use the algebraically equivalent direct interpolation rather
    than explicitly forming ``(x_hat - x_t) / (1 - t)``.  This avoids a
    division near the clean-action endpoint and makes the final step exactly
    equal to the predicted clean action.
    """
    dt = next_time - time
    if prediction_type == "v_prediction":
    ...
    if prediction_type in ("x_prediction", "x_prediction_v_loss"):
        ...
        if remaining <= 0:
            return model_output
        return ((1.0 - next_time) / remaining) * actions + (dt / remaining) * model_output
    raise ValueError(f"Unsupported canonical prediction_type={prediction_type!r}")
...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        # v_prediction is the released GR00T behaviour.  Both x-pred modes
        # output a clean action at t=1.  x_prediction uses direct action MSE;
        # x_prediction_v_loss is ABot AML's clean-action output plus v-MSE.
        self.prediction_type = normalize_prediction_type(config.get("prediction_type", "v_prediction"))
        # ABot AML clamps 1 - t in its velocity conversion to avoid an
        # unbounded low-noise loss weight.  Its local implementation defaults
        # to 5e-2; expose the same value in YAML for reproducible experiments.
        self.t_eps = float(config.get("t_eps", 5e-2))
    ...
    def predict_action(
        ...
        for t in range(num_steps):
            ...
            next_t_cont = (t + 1) / float(num_steps)
            actions = flow_matching_step(
                actions, pred_actions, t_cont, next_t_cont, self.prediction_type
            )
        return actions
