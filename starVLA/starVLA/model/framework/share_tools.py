...
def read_mode_config(pretrained_checkpoint):
    ...
    if os.path.isfile(pretrained_checkpoint):
        ...
        # Training writes a compact user-facing ``config.yaml`` as well as the
        # resolved ``config.full.yaml``.  The compact file intentionally omits
        # trainer/runtime fields, so it cannot reconstruct a framework from a
        # checkpoint.  Prefer the resolved snapshot and retain the compact file
        # only for older runs that predate ``config.full.yaml``.
