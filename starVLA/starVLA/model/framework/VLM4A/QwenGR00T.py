...
class QwenGR00TDefaultConfig:
    ...
    action_model: dict = field(
        default_factory=lambda: {
            ...
            # Output parameterisation for the action DiT.  v_prediction is
            # the released GR00T baseline; x_prediction regresses clean actions.
...
class Qwen_GR00T(baseframework):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Qwen3-VL stores the language hidden size in ``text_config`` whereas
        # Qwen2.5-VL exposes it on the top-level model config.  The GR00T
        # cross-attention must use that language hidden size in either case.
        vlm_hf_cfg = self.qwen_vl_interface.model.config
        text_cfg = getattr(vlm_hf_cfg, "text_config", vlm_hf_cfg)
        vl_hidden_dim = int(text_cfg.hidden_size)
