...
from .utils.monitoring import write_histogram_svg, write_loss_curve_svg
...
class Wan22Trainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        ...
        self.monitoring_enabled = bool(monitoring_cfg.get("enabled", True))
        self.monitoring_curve_every = max(int(monitoring_cfg.get("curve_every", self.log_every)), 1)
        self.monitoring_max_curve_points = max(int(monitoring_cfg.get("max_curve_points", 2000)), 1)
        self.action_error_hist_bins = max(int(monitoring_cfg.get("action_error_hist_bins", 40)), 2)
        self.save_action_error_arrays = bool(monitoring_cfg.get("save_action_error_arrays", True))
        self.metrics_path = Path(self.output_dir) / "metrics.jsonl"
        self.loss_curve_path = Path(self.output_dir) / "loss_curve.svg"
        self.action_error_dir = Path(self.eval_dir) / "action_errors"
        self._metric_history = []
        self._metrics_file = None
        if self.monitoring_enabled and self.accelerator.is_main_process:
            self.action_error_dir.mkdir(parents=True, exist_ok=True)
            if self.metrics_path.exists():
                for line in self.metrics_path.read_text(encoding="utf-8").splitlines():
                    try:
                        self._metric_history.append(json.loads(line))
                    except json.JSONDecodeError:
                        logger.warning("Ignoring malformed metric record in %s", self.metrics_path)
    ...
    def _record_metrics(self, kind: str, payload: dict) -> None:
        """Append machine-readable main-process metrics and refresh the local loss chart."""
        if not self.monitoring_enabled or not self.accelerator.is_main_process:
            return
        record = {
            "kind": kind,
            "timestamp": time.time(),
        ...
        if self._metrics_file is not None:
            self._metrics_file.write(json.dumps(record, ensure_ascii=True, allow_nan=False) + "\n")
            self._metrics_file.flush()
        if kind == "eval" or self.global_step % self.monitoring_curve_every == 0:
            write_loss_curve_svg(
                self._metric_history,
                self.loss_curve_path,
                max_points=self.monitoring_max_curve_points,
            )
    ...
    def evaluate(self):
        ...
        if self.accelerator.device.type == "cuda":
            torch.cuda.synchronize(self.accelerator.device)
        rollout_infer_latency_ms = (time.perf_counter() - rollout_start) * 1000.0
        
        pred_video = pred["video"]
        # Measure the policy path separately from video rollout. This is the latency
        # relevant to a control loop and supplies actions for error/smoothness metrics.
        action_infer_kwargs = {
            "prompt": None if sample["context"] is not None else prompt,
            "input_image": input_image,
            "action_horizon": sample["action_horizon"],
            "num_video_frames": num_frames,
            "proprio": proprio,
            "context": sample["context"][0] if sample["context"] is not None else None,
            "context_mask": sample["context_mask"][0] if sample["context"] is not None else None,
        ...
        action_params = inspect.signature(model.infer_action).parameters
        action_infer_kwargs = {key: value for key, value in action_infer_kwargs.items() if key in action_params}
        if self.accelerator.device.type == "cuda":
            torch.cuda.synchronize(self.accelerator.device)
        action_start = time.perf_counter()
        action_out = model.infer_action(**action_infer_kwargs)
        if self.accelerator.device.type == "cuda":
            torch.cuda.synchronize(self.accelerator.device)
        action_infer_latency_ms = (time.perf_counter() - action_start) * 1000.0
        pred_action = action_out.get("action", None)
        ...
        action_jerk = None
        gt_action_smoothness = None
        gt_action_jerk = None
        action_error_p50 = None
        action_error_p90 = None
        action_error_p95 = None
        if action is not None and pred_action is not None:
            ...
            abs_action_error = action_diff.abs().detach().cpu().numpy().reshape(-1)
            action_error_p50, action_error_p90, action_error_p95 = (
                float(value) for value in np.percentile(abs_action_error, [50, 90, 95])
            )

            # First and second finite differences of physical action commands.
            # Lower values mean smoother command sequences, but should be interpreted
            # alongside task success because overly smooth actions can be under-responsive.
            pred_delta = pred_action_denorm[:, 1:] - pred_action_denorm[:, :-1]
            gt_delta = gt_action_denorm[:, 1:] - gt_action_denorm[:, :-1]
            if pred_delta.shape[1] > 0:
                action_smoothness = torch.linalg.vector_norm(pred_delta, dim=-1).mean().item()
                gt_action_smoothness = torch.linalg.vector_norm(gt_delta, dim=-1).mean().item()
            if pred_delta.shape[1] > 1:
                pred_jerk = pred_delta[:, 1:] - pred_delta[:, :-1]
                gt_jerk = gt_delta[:, 1:] - gt_delta[:, :-1]
                action_jerk = torch.linalg.vector_norm(pred_jerk, dim=-1).mean().item()
                gt_action_jerk = torch.linalg.vector_norm(gt_jerk, dim=-1).mean().item()

            if self.monitoring_enabled and self.accelerator.is_main_process:
                error_stem = self.action_error_dir / f"step_{self.global_step:06d}"
                write_histogram_svg(
                    abs_action_error,
                    error_stem.with_suffix(".svg"),
                    bins=self.action_error_hist_bins,
                )
                if self.save_action_error_arrays:
                    np.savez_compressed(
                        error_stem.with_suffix(".npz"),
                        absolute_error=abs_action_error,
                        signed_error=action_diff.detach().cpu().numpy(),
                        pred_action=pred_action_denorm.detach().cpu().numpy(),
                        gt_action=gt_action_denorm.detach().cpu().numpy(),
                    )
        ...
        local_metrics = torch.tensor(
            [
                ...
                float(rollout_infer_latency_ms),
                float(action_infer_latency_ms),
                float(action_smoothness) if action_smoothness is not None else -1.0,
                float(action_jerk) if action_jerk is not None else -1.0,
                float(gt_action_smoothness) if gt_action_smoothness is not None else -1.0,
                float(gt_action_jerk) if gt_action_jerk is not None else -1.0,
            ],
        ...
        rollout_infer_latency_ms_mean = gathered_metrics[:, 9].mean().item()
        action_infer_latency_ms_mean = gathered_metrics[:, 10].mean().item()
        action_smoothness_mean = gathered_metrics[:, 11].mean().item() if action_smoothness is not None else None
        action_jerk_mean = gathered_metrics[:, 12].mean().item() if action_jerk is not None else None
        gt_action_smoothness_mean = gathered_metrics[:, 13].mean().item() if gt_action_smoothness is not None else None
        gt_action_jerk_mean = gathered_metrics[:, 14].mean().item() if gt_action_jerk is not None else None
        ...
        result = {
            ...
            "rollout_infer_latency_ms": float(rollout_infer_latency_ms_mean),
            "action_infer_latency_ms": float(action_infer_latency_ms_mean),
        ...
        if action_smoothness_mean is not None:
            result["action_smoothness"] = float(action_smoothness_mean)
        if action_jerk_mean is not None:
            result["action_jerk"] = float(action_jerk_mean)
        if gt_action_smoothness_mean is not None:
            result["gt_action_smoothness"] = float(gt_action_smoothness_mean)
        if gt_action_jerk_mean is not None:
            result["gt_action_jerk"] = float(gt_action_jerk_mean)
        if action_error_p50 is not None:
            result["action_error_p50"] = action_error_p50
            result["action_error_p90"] = action_error_p90
            result["action_error_p95"] = action_error_p95
        self._record_metrics("eval", {key: value for key, value in result.items() if key != "video_path"})
        return result
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if (
                        ...
                        if metrics is not None and self.accelerator.is_main_process:
                            ...
                            description += " action_ms=%.1f rollout_ms=%.1f" % (
                                metrics["action_infer_latency_ms"],
                                metrics["rollout_infer_latency_ms"],
                            )
                            if "action_smoothness" in metrics:
                                description += " action_smooth=%.4f" % metrics["action_smoothness"]
                            logger.info(description)
                            eval_payload = {
                            ...
                            for key in (
                                "action_smoothness",
                                "action_jerk",
                                "gt_action_smoothness",
                                "gt_action_jerk",
                                "action_error_p50",
                                "action_error_p90",
                                "action_error_p95",
                                "rollout_infer_latency_ms",
                                "action_infer_latency_ms",
                            ):
                                if key in metrics:
