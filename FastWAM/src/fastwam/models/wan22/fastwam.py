...
from .mot import MoT
from .schedulers.scheduler_continuous import (
...
class FastWAM(torch.nn.Module):
    ...
    def __init__(
        ...
        action_prediction_type: str = "v_prediction",
        action_x_pred_v_loss_eps: float = 0.05,
    ):
        super().__init__()
        ...
        self.action_prediction_type = normalize_action_prediction_type(action_prediction_type)
        self.action_x_pred_v_loss_eps = float(action_x_pred_v_loss_eps)
        if self.action_x_pred_v_loss_eps <= 0:
            raise ValueError(
                "`action_x_pred_v_loss_eps` must be positive, "
                f"got {self.action_x_pred_v_loss_eps}."
            )
    ...
    def from_wan22_pretrained(
        ...
        model = cls(
            ...
            action_x_pred_v_loss_eps=action_x_pred_v_loss_eps,
        )
        model.model_paths = {
    ...
    def training_loss(self, sample, tiled: bool = False):
        ...
        if self.action_prediction_type == "v_prediction":
    ...
    def infer_joint(
        ...
        for step_t_video, step_delta_video, step_t_action, step_delta_action in zip(
            ...
            latents_action = action_prediction_step(
                sample=latents_action,
                prediction=pred_action,
                timestep=timestep_action,
                delta=step_delta_action,
