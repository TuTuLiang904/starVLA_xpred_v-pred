...
bundled_libero_root = project_root / "third_party" / "LIBERO"
if bundled_libero_root.exists():
    # The official repository uses a namespace-package layout, so adding its
    # checkout root is more reliable than relying on a PEP 660 editable install.
    sys.path.insert(0, str(bundled_libero_root))
    os.environ["PYTHONPATH"] = str(bundled_libero_root) + os.pathsep + os.environ.get("PYTHONPATH", "")
    os.environ.setdefault("LIBERO_CONFIG_PATH", str(project_root / "third_party" / "libero_config"))
    ...
    env.update(
        {
            ...
            # tmux panes source ~/.bashrc, so never rely on its `python` PATH.
