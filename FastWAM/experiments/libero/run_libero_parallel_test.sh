...
run_libero_eval() {
    ...
    export ROOT_DIR
    if [ -d "$ROOT_DIR/third_party/LIBERO" ]; then
        export PYTHONPATH="$ROOT_DIR/third_party/LIBERO${PYTHONPATH:+:$PYTHONPATH}"
        export LIBERO_CONFIG_PATH="${LIBERO_CONFIG_PATH:-$ROOT_DIR/third_party/libero_config}"
    fi
    ...
    if [ "$(realpath "$task_list_file")" != "$(realpath "$OUTPUT_DIR/$(basename "$task_list_file")")" ]; then
    ...
    launch_task_on_pane() {
        ...
        tmux send-keys -t $SESSION_NAME:$pane_info "source ~/.bashrc && cd $ROOT_DIR && export EXP_NAME=$EXP_NAME && \
            ...
            CUDA_VISIBLE_DEVICES=$gpu_id \"$PYTHON_EXECUTABLE\" experiments/libero/eval_libero_single.py \
