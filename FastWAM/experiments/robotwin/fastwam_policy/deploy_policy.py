...
def _checkpoint_action_prediction_type(checkpoint_path: str, configured_value: Any) -> tuple[str, Optional[Path]]:
    """Prefer the parameterization recorded alongside a training checkpoint.

    A FastWAM checkpoint stores weights only, not the Hydra model setting.  A
    clean-action checkpoint loaded with the default v-pred config therefore
    produces invalid rollout actions.  Training runs save config.yaml two
    directories above ``checkpoints/weights/*.pt``; released checkpoints may
    not have this companion file, in which case the explicitly composed config
    remains the backwards-compatible fallback.
    """
    config_path = Path(checkpoint_path).expanduser().resolve().parents[2] / "config.yaml"
    if not config_path.is_file():
        return str(configured_value), None
    saved_cfg = OmegaConf.load(config_path)
    saved_value = OmegaConf.select(saved_cfg, "model.action_prediction_type")
    if saved_value is None:
        raise ValueError(f"model.action_prediction_type is missing from training config: {config_path}")
    return str(saved_value), config_path
...
class WorldActionRobotWinPolicy:
    def __init__(
        ...
        logger.info(
            ...
            "| action_prediction_type=%s",
            checkpoint_path,
            dataset_stats_path,
            self.action_horizon,
            self.replan_steps,
            self.model.action_prediction_type,
        )
...
def get_model(usr_args: Dict[str, Any]):
    ...
    action_prediction_type, training_config_path = _checkpoint_action_prediction_type(
        checkpoint_path=str(checkpoint_path),
        configured_value=cfg.model.action_prediction_type,
    )
    OmegaConf.update(cfg, "model.action_prediction_type", action_prediction_type, merge=False)
    if training_config_path is None:
        logger.warning(
            "No adjacent training config found for checkpoint=%s; using composed "
            "action_prediction_type=%s.",
            checkpoint_path,
        ...
        logger.info(
            "Using action_prediction_type=%s from training config %s.",
            action_prediction_type,
