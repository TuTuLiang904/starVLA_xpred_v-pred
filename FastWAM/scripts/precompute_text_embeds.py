...
def _collect_dataset_settings(data_cfg: DictConfig):
    ...
    episode_subset_paths: dict[str, Path | None] = {}
    ...
    for node_path, node in _iter_dataset_nodes(data_cfg, path="data"):
        ...
        for ds in raw_dirs:
            # Train/val configs may spell the same directory with or without a
            # trailing slash. Canonicalize it so a shared subset is scanned once.
            ds_str = str(Path(str(ds)).expanduser().resolve())
            subset_raw = node.get("episode_subset_path")
            subset_path = None if subset_raw is None or not str(subset_raw).strip() else Path(str(subset_raw)).expanduser()
            if ds_str in episode_subset_paths and episode_subset_paths[ds_str] != subset_path:
                raise ValueError(
                    f"Dataset {ds_str} has conflicting episode_subset_path values: "
                    f"{episode_subset_paths[ds_str]} vs {subset_path}"
                )
            episode_subset_paths[ds_str] = subset_path
    ...
    return dataset_dirs, episode_subset_paths, cache_dirs, context_lens
...
def _read_unique_prompts(dataset_dirs: list[str], episode_subset_paths: dict[str, Path | None]) -> list[str]:
    prompts: list[str] = []
    seen = set()
    total_task_rows = 0

    for ds_dir in dataset_dirs:
        subset_path = episode_subset_paths.get(ds_dir)
        if subset_path is None:
            tasks_path = Path(ds_dir) / "meta" / "tasks.jsonl"
            if not tasks_path.exists():
                raise FileNotFoundError(f"Missing tasks file: {tasks_path}")
            records = ((line_idx, json.loads(line)) for line_idx, line in enumerate(tasks_path.open(encoding="utf-8"), start=1) if line.strip())
        else:
            if not subset_path.is_file():
                raise FileNotFoundError(f"Episode subset manifest not found: {subset_path}")
            with subset_path.open(encoding="utf-8") as f:
                subset_payload = json.load(f)
            episode_indices = subset_payload.get("episode_indices", subset_payload) if isinstance(subset_payload, dict) else subset_payload
            if not isinstance(episode_indices, list):
                raise ValueError(f"episode_indices must be a JSON list in {subset_path}")
            ...
            logger.info("Reading prompts from %d selected complete episodes in %s.", len(selected), subset_path)
            records = (
                (line_idx, task)
                for line_idx, line in enumerate(episodes_path.open(encoding="utf-8"), start=1)
                if line.strip()
                for record in [json.loads(line)]
                if int(record["episode_index"]) in selected
                for task in record.get("tasks", [])
            )

        for line_idx, record in records:
            task = record.get("task") if isinstance(record, dict) else record
            if task is None:
                raise KeyError(f"Missing task text at {ds_dir}:{line_idx}")
...
def main(cfg: DictConfig):
    ...
    dataset_dirs, episode_subset_paths, cache_dirs, context_lens = _collect_dataset_settings(cfg.data)
    if not cache_dirs:
    ...
    if override_prompt is not None:
        ...
        prompts = _read_unique_prompts(dataset_dirs, episode_subset_paths)
    if not prompts:
